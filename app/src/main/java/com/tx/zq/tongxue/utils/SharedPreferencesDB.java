...
public class SharedPreferencesDB {
	...
	private SharedPreferencesDB(Context context) {
		...
		preferences = this.context.getSharedPreferences(TZ+ VersionUtils.getVersionName(context), 0);
	}
	 ...
	 * @param data
	 */
	//public void saveLoginUserinfo(TongZhuanInfo data, boolean isUpdateinfo) {
		// String leavl = data.getUserRank() + "";
		// String exp = data.getUserExperience() + "";
		// String score = data.getUserScores() + "";
		//
		// // 仅修改积分
		// if (!isUpdateinfo) {
		// setString(TongZhuanUserInfo.P_USER_LEAVL, leavl);
		// setString(TongZhuanUserInfo.P_USER_EXP, exp);
		// setString(TongZhuanUserInfo.P_USER_SCORES, score);
		// } else {
